...
#define V_LOGF(logger, level, format_str, ...)                                  \
  if (::onnxruntime::logging::vlog_enabled && level <= (logger).VLOGMaxLevel()) \
...
#define VLOGS_DEFAULT(level) \
  LOGS(::onnxruntime::logging::LoggingManager::DefaultLogger(), level)
...
#define VLOGF_DEFAULT(level, format_str, ...) \
  V_LOGF(::onnxruntime::logging::LoggingManager::DefaultLogger(), level, format_str, ##__VA_ARGS__)
