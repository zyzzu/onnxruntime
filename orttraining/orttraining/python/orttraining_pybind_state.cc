...
void addObjectMethodsForTraining(py::module& m) {
  ...
  parameters.def(py::init())
      ...
      .def("set_optimizer_initial_state",
           [](TrainingParameters& parameters, std::unordered_map<std::string, std::unordered_map<std::string, py::object>>& py_state) -> void {
             onnxruntime::training::TrainingSession::OptimizerState optim_state;
             for (auto weight_it : py_state) {
               ...
               optim_state.insert(std::make_pair(weight_it.first, state_tensors));
             }
  ...
  training_session
      ...
      .def("get_model_state", [](PyTrainingSession* sess, bool incl_mp_weights) {
        ...
        for (auto& kv1 : model_state_tensors) {
      ...
      .def("load_model_opt_state", [](PyTrainingSession* sess, std::unordered_map<std::string, py::object>& model_state, std::unordered_map<std::string, std::unordered_map<std::string, py::object>>& opt_state, bool strict) {
        NameMLValMap model_tensors;
        for (auto initializer : model_state) {
          OrtValue ml_value;
          auto px = sess->GetSessionHandle()->GetModelInputs();
          if (!px.first.IsOK() || !px.second) {
            throw std::runtime_error("Either failed to get model inputs from the session object or the input def list was null");
        ...
        ORT_THROW_IF_ERROR(static_cast<TrainingSession*>(sess->GetSessionHandle())->SetModelOptState(model_tensors, opt_tensors, strict));
      })
