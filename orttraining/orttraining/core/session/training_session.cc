...
Status TrainingSession::ConfigureForTraining(
  ...
  ORT_RETURN_IF_ERROR(BuildGradientGraph(weights_to_train_, loss_name, config.gradient_graph_config, *session_logger_));
  if (config.pipeline_config.has_value()) {
    TrainingConfigurationResult::PipelineConfigurationResult pipeline_result{};
    ORT_RETURN_IF_ERROR(InsertPipelineOps(weights_to_train_,
