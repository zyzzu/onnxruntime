...
static void UnpackTensorWithRawData(const void* raw_data, size_t raw_data_length, size_t expected_size,
  ...
  if (!CalcMemSizeForArrayWithAlignment(expected_size, sizeof(T), 0, &expected_size_in_bytes)) {
    abort();  // throw Ort::Exception("size overflow", OrtErrorCode::ORT_FAIL);
  }
  if (raw_data_length != expected_size_in_bytes)
    abort();  // throw Ort::Exception(MakeString("UnpackTensor: the pre-allocated size does not match the raw data size, expected ",
              //           expected_size_in_bytes, ", got ", raw_data_length),
              //OrtErrorCode::ORT_FAIL);
  if (endian::native != endian::little) {
    abort();  // throw Ort::Exception("UnpackTensorWithRawData only handles little-endian native byte order for now.",
...
void UnpackTensor(const onnx::TensorProto& tensor, const void* /*raw_data*/, size_t /*raw_data_len*/,
  ...
  if (nullptr == p_data) {
    ...
    abort();  // throw Ort::Exception("", OrtErrorCode::ORT_INVALID_ARGUMENT);
  }
  ...
  if (tensor.string_data_size() != expected_size)
    abort();  // throw Ort::Exception(        "UnpackTensor: the pre-allocate size does not match the size in proto", OrtErrorCode::ORT_FAIL);
...
void UnpackTensor(const onnx::TensorProto& tensor, const void* raw_data, size_t raw_data_len,
  ...
  for (int i = 0; i < static_cast<int>(expected_size); i++) {
    ...
    if (v < 0 || v > max_value) {
      abort();  // throw Ort::Exception(          "data overflow", OrtErrorCode::ORT_FAIL);
    }
