  ...
  */
  ...
  Status GeneratePlanForWeights() {
    ...
    for (auto& node : graph_viewer_.Nodes()) {
      auto lambda = [this, &locations, &node, &weights](const onnxruntime::NodeArg& def, size_t index) {
        ORT_TRY {
      ...
      ORT_RETURN_IF_ERROR(onnxruntime::Node::ForEachWithIndex(node.InputDefs(), lambda));
    }
