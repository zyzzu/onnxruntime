...
class MemPatternPlanner {
 public:
  // Dirty hack as WIP. Would be better to wire up something from SessionState -> OrtValuePatternPlanner -> here to
  // specify if the TraceAllocate calls will be using program counters
#ifndef ENABLE_TRAINING
  MemPatternPlanner() : using_counters_{false} {
  }

#else
  // only the Training code currently uses the program counter based logic
  MemPatternPlanner() : using_counters_{true} {}
  ...
  bool
  OverlappingTimeSchedules(const AllocPlanPerValue::ProgramCounter& counter1,
  ...
  //
  // FIXME: This setup could be improved. There are two TraceAllocation methods, but it's only valid to use one
  // otherwise you end up with entries that may or may not have the counter. There should possibly be two variants
  // or MemoryPatternPlanner so the interface is clearer. Validating with using_counters_ for the moment.
  //
