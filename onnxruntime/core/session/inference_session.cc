...
common::Status InferenceSession::RegisterExecutionProvider(std::unique_ptr<IExecutionProvider> p_exec_provider) {
  ...
  // V_LOGS(*session_logger_, 1) << "Adding execution provider of type: " << provider_type;
...
common::Status InferenceSession::Load(const std::basic_string<T>& model_uri) {
  ...
  common::Status
  InferenceSession::Load(const std::string& model_uri) {
    if (model_loaded_) {
  ...
  return Load(loader, "model_loading_proto");
    ...
    common::Status
    InferenceSession::Load(std::unique_ptr<ModelProto> p_model_proto) {
      if (model_loaded_) {
        return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                               "ModelProto corresponding to the model to be loaded has already been parsed. "
                               "Invoke Load().");
  ...
  return Load(loader, "model_loading_proto");
      ...
      common::Status
      InferenceSession::Load(std::istream & model_istream) {
        if (model_loaded_) {
          return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                                 "ModelProto corresponding to the model to be loaded has already been parsed. "
                                 "Invoke Load().");
  ...
  return Load(loader, "model_loading_istream");
        ...
        common::Status
        InferenceSession::Load(const void* model_data, int model_data_len) {
          if (model_loaded_) {
            return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                                   "ModelProto corresponding to the model to be loaded has already been parsed. "
                                   "Invoke Load().");
  ...
  return Load(loader, "model_loading_from_saved_proto");
            ...
            common::Status
            InferenceSession::TransformGraph(onnxruntime::Graph & graph, const onnxruntime::GraphTransformerManager& graph_transformer_mgr, const ExecutionProviders& providers, KernelRegistryManager& kernel_registry_manager, const InsertCastTransformer& insert_cast_transformer, SessionState& session_state) {
              ...
              if (is_verbose_mode) {
                ...
                if (node_placements.size() == 1) {
                  ...
                  for (const auto& pr : node_placements) {
                    ...
                    // LOGS(*session_logger_, VERBOSE) << " Provider: [" << pr.first << "]"                    << ": [" << all_nodes_str.str() << "]";
                  }
                }
              }
  ...
  auto expected_name = std::string(typeid(*expected).name());
            ...
            common::Status InferenceSession::SaveModelMetadata(const onnxruntime::Model& model) {
              // V_LOGS(*session_logger_, 1) << "Saving model metadata";
              ...
              // V_LOGS(*session_logger_, 1) << "Done saving model metadata";
              return common::Status::OK();
            }
            ...
            const logging::Logger& InferenceSession::CreateLoggerForRun(const RunOptions& run_options,
              ...
              if (logging_manager_ != nullptr) {
                ...
                // V_LOGS(*run_logger, 1) << "Created logger for run with id of " << run_log_id;
              } else {
                // fallback to using default logger. this does NOT have any session or run specific id/tag in it
                run_logger = session_logger_;
                // V_LOGS(*run_logger, 1) << "Using default logger for run " << run_options.run_tag;
              }
