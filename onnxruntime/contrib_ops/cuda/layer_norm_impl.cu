...
#define LAYERNORM_LINEAR_IMPL(T, U, simplified)                                                                       \
  template void HostApplyLayerNorm<T, U, simplified>(const cudaDeviceProp& prop, cudaStream_t stream, T* output, U* mean, U* invvar, const T* input, int64_t n1, int64_t n2, \
